...
- **Пробел в описании роли (нет владельца у работы)** — сюда ТОЛЬКО если пробел уже держит конкретную работу в очереди или срывает срок (эффект на поток подтверждён). Эффект не подтверждён → сначала `/org-dev` (R31): вопрос владения на уровне устройства организации, не узкое место конвейера. Критерий развилки зафиксирован в обоих скиллах (WP-481 Ф7 п.6, пир-сессия с Codex 11.08.2026).
...
|--------|------------------|---------------------|
| `WP-NNN` | `DS-my-strategy/inbox/WP-NNN-*.md` + связанные РП + git за 7d | governance |
| `weekplan` | `DS-my-strategy/current/WeekPlan W{N}.md` + active-wp.md + DP.SC поверх активных РП | governance |
...
## Algorithm — пятифазный ВДВ-каскад
...
**Выход:** **Stage Dependency Map** (формат DP.WP.016) — в чате как markdown + опционально в `DS-my-strategy/inbox/WP-NNN/stage-map.md`:
...
Сохранить в `DS-my-strategy/inbox/bottleneck-pick-runs/<YYYY-MM-DD>-<target>.yaml`:
