...
**Обязательная трассировка при косяке из неоднозначной формулировки (WP-481 №6, пятый семинар FPF).** «Агенты не ошибаются просто так» — если косяк вызван тем, что правило/инструкция допускала двоякое толкование (не банальная невнимательность), агент обязан НЕ ограничиваться `--fault`, а: (1) процитировать точную формулировку-источник через `--source-citation` (найти конкретную фразу в CLAUDE.md/rules/skill, которая спровоцировала ошибку); (2) предложить правку этой формулировки прямо в описании `--fault` (не обязательно применять сразу — зафиксировать предложение как часть записи). Починка формулировки, а не инстанса — цель записи, не факт инцидента сам по себе.
