...
**Актуализация направлений (РП7 Ф156, 17.09.2026).** Строка `⚠️ N РП плана недели не отнесены ни к одному направлению` — не только предупреждение, задача для агента прямо на этом шаге, до дублирования в «Требует внимания»: для каждого перечисленного РП прочитать его карточку (`Проблема`/`Артефакт`) и цели всех направлений (`current/WeekPlan W{N}.md` §«Направления недели»). Ровно одно направление явно подходит по цели (не по типу работы — инфраструктура тоже может быть «для новичка», если её единственная цель — канал входа новичка, прецедент РП-560/РП-581) → узким `Edit` дописать номер РП в ячейку РП этого направления. Никакая эвристика без причины не считается: скрипт `day-close-weekstate-fill.py` намеренно не делает такого сопоставления сам (WHY в его docstring — риск галлюцинации), поэтому подстановка руками агента здесь обязана иметь явное обоснование одной строкой в отчёте дня, не молчаливую догадку. Нет однозначного направления (несколько кандидатов, ни одного, или цели направлений сами конфликтуют) → не гадать: оставить обычную строку-дубликат в «Требует внимания», как раньше. Это касается только ячейки «РП» направления — «Осталось», приоритет и статус самой карточки РП не трогать; масштаб «Неделя», РП Gate не требуется (`protocol-open.md §Неделя`).
