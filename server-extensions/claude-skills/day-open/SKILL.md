...
Day Open = протокол. Исполнять ТОЛЬКО пошагово через `task_tracker.create` (Host Contract DP.IWE.011 §5).
Каждый шаг алгоритма ниже → отдельная единица отслеживания (pending → in_progress → completed).
Переход к следующему — ТОЛЬКО после отметки текущего. Шаг невозможен → blocked (не пропускать молча).
**Почему:** без явного пошагового трекера с обязательным обновлением статуса агент пропускает шаги из-за загрязнения контекста (SOTA.002). Конкретный инструмент резолвится адаптером по `$IWE_RUNTIME` — CC-адаптер `claude-code-adapter.md`, headless-адаптер `DP.IWE.011-adapters/headless-adapter.md` — с тем же требованием переходов статусов, не называется здесь напрямую.
...
### 1c. Заметки — решает ТОЛЬКО пилот (БЛОКИРУЮЩЕЕ, 30.07.2026, WP-484 Ф29)
`DS-my-strategy/inbox/fleeting-notes.md` → агент готовит ТОЛЬКО предложения (Тип + Предложение) в секцию «Разбор заметок» и ждёт решения пилота. До явного решения ЗАПРЕЩЕНО: снимать bold, архивировать, удалять, писать по заметке в inbox/captures/ / WeekPlan / Dissatisfactions. Ночной авторазбор (Note-Review 23:00) отключён 30.07.2026 — «git log note-review показывает обработку» аргументом больше не является.
**Carry-over (БЛОКИРУЮЩЕЕ):** заметка без записанного решения пилота попадает в КАЖДЫЙ следующий DayPlan — без срока давности. «Все обработаны» писать можно ТОЛЬКО если по каждой заметке в `archive/notes/Notes-Archive.md` есть запись «**Разбор:** дата — **Решение пилота:** …» (дать на неё ссылку).
**Журнал решений (БЛОКИРУЮЩЕЕ):** решение пилота исполняется сразу и фиксируется в `archive/notes/Notes-Archive.md`: дата разбора + решение пилота + что сделано. Только после этой записи заметка убирается из fleeting-notes.md.
**Гиперссылки на заметки (БЛОКИРУЮЩЕЕ):** каждая заметка в секции «Разбор заметок» DayPlan — markdown-ссылка на источник (`inbox/fleeting-notes.md` для ждущих решения, `archive/notes/Notes-Archive.md#L<line>` для решённых). Формат строки таблицы: `[«заголовок»](путь#L<line>) (DD мес HH:MM)`.
**Знаниевые заметки = кандидаты (БЛОКИРУЮЩЕЕ, 2026-04-17):** заметки категории «Знание доменное» без явного маркера «Экстрактору» в тексте → в DayPlan секция «Разбор заметок» таблицей **Кандидаты Экстрактору** с колонками «Заметка | Тип | Предполагаемый Pack | Действие». Решение «отдать / оставить» принимает пользователь в живом разборе. Note-Review в `inbox/captures/<текущий месяц>.md` пишет ТОЛЬКО при явном маркере (легаси `inbox/captures.md` — архив с 29.08.2026, WP-526, только для чтения). Причина: очередь Экстрактора = любое знание туда = неявное согласие на формализацию, которое Note-Review делать не уполномочен.

### 1d. Обновить срез РП в MEMORY.md (WP-484 Ф1, 2026-09-10)

`Текущая работа` в MEMORY.md — сгенерированный срез, не ручная таблица (DRR: `DS-my-strategy/inbox/WP-484/DRR-memory-index-generated-slice.md`). Регенерировать перед шагом 2 (он читает эту секцию как вход):

```bash
python3 "${IWE_WORKSPACE:-$HOME/IWE}/DS-my-strategy/scripts/wp-today-slice.py" \
  "${IWE_WORKSPACE:-$HOME/IWE}/DS-my-strategy/docs/WP-REGISTRY.md" --window-days 2
```

Заменить содержимое таблицы (между строкой `| WP | ст | P | Название |` и следующим `## `) на вывод — узкий Edit, не полный Write (контракт `memory-lifecycle-spec.md §7.5`). Скрипт упал/дал пустой вывод → не затирать существующую таблицу, оставить как есть и отметить в «Требует внимания».

### 1e. Направления недели (WP-561 Ф14, 2026-09-15) — секция «Направления сегодня»

Отдельный такт координации: несколько направлений работы внутри недели не должны двигаться вслепую друг от друга. `day-open-scaffold.sh` сам вызывает `day-open-weekstate-render.py` (без участия агента, не PENDING-секция — `day-open-llm-fill.py` её не трогает по своему инварианту) и печатает готовую секцию «Направления сегодня» из `current/WeekState W{N}.md` (PIPE-17, генерирует закрытие дня). Направления в плане недели не заданы, файл отсутствует или устарел (>1 дня) → секция это честно показывает, DayPlan строится как обычно (WeekPlan + перенос со вчера) — эта секция никогда не блокирует шаг 2. Строки `⚠️`/`🗓️` из WeekState scaffold дублирует в «Требует внимания» (6b) как «направления недели: …» — секция направлений свёрнута, а «Требует внимания» доходит до compact dashboard (РП-561 Ф15).
...
### 3. Саморазвитие
Ссылка на личное руководство — врезается кодом (`inject_guide_link()`, WP-521), вручную не вписывать. Заполнить только «где остановился», черновики (`DS-Tseren-Brand/content/posts/`).
