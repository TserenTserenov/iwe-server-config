...
## Шаг 0б. Типизированный маршрут по kind-у результата (WP-481 Ф7, только `auto`/пусто)

> **Приоритет явного режима — абсолютный.** Если пилот/агент буквально НАПИСАЛ режим первым токеном `$ARGUMENTS` (`code`, `archgate`, `capture`, `pack`, `wp`, `chain`, `adversarial`, `subsection`, `section`, `guide`, `ontological`) — использовать его КАК ЕСТЬ, пропустить весь этот шаг целиком. Это сохраняет неизменным горячий путь `pack-creator → /verify ontological` (встроен как обязательный проход с 04.08.2026, WP-474 Ф10) — типизированный диспетчер его не трогает.

**Порядок относительно Шага 0 (снимает найденную неоднозначность, ревью 05.08.2026): Шаг 0б проверяется РАНЬШЕ автоопределения Шага 0, не после.** Автоопределение Шага 0 (в т.ч. правило «путь к WP context → `wp`») — эвристика по паттерну пути, а не буквально введённый режим; она НЕ считается «явно заданным режимом» для правила абзаца выше. Порядок разбора вызова `/verify [артефакт]` без буквенного режима: (1) есть источник `result_kind` у артефакта → идти по этому шагу целиком, автоопределение Шага 0 не запускается; (2) источника `result_kind` нет → откатиться к автоопределению Шага 0 как раньше (в т.ч. `wp` для карточки РП без размеченного kind-а). Типизированные данные конкретнее эвристики по пути — там, где они есть, они решают первыми.

Применяется только когда режим не задан или равен `auto`, И у проверяемого артефакта есть источник `result_kind` (frontmatter карточки РП, контракт передачи агенту, явный аргумент). Источник реестра: `PACK-digital-platform/pack/digital-platform/result-kinds-registry.yaml` (kind → required_structure/handler_type/handler_id/required_lenses/aggregation_policy/gate_ready).

**Источники и приоритет `expected` / `actual`:**
- `expected_result_kind` — из контракта передачи (frontmatter поля `result_kind:` на карточке РП, либо `expected_result_kind` из вывода `/artifactor`, если классификация происходила в этой же сессии).
- `actual_result_kind` — из самого проверяемого артефакта (его собственное поле `result_kind:`, если оно есть) либо явного аргумента вызывающего.
- **`actual` отсутствует** → verdict `NOT_VERIFIABLE` («артефакт не размечен result_kind — не проверен, не провален»), остальные шаги этого раздела не выполняются. Не считать отсутствие `actual` автоматическим совпадением с `expected`.

**Последовательность (каждый шаг может завершить проверку раньше срока):**
1. **Membership.** `actual_result_kind` не входит в реестр → `NOT_VERIFIABLE` с диагностикой «неизвестный kind, не generic/other» (реестр запрещает подмену ближайшим).
2. **Сверка expected/actual.** `expected != actual` → как правило `FAIL` до запуска handler («результат не того типа, который ожидался»). **Единственное исключение:** `actual == NegativeResult` — несовпадение само по себе НЕ даёт `FAIL` (честный отрицательный результат — валидное продвижение, FPF семинар 1, п.5/58); переходит к шагу 3 как обычно.
3. **`gate_ready`.** `false` у kind-а → `NOT_VERIFIABLE` с текстом `reason_not_ready` из реестра (артефакт технически не проверен — путь проверки не операционализирован, не провален).
4. **`required_structure`.** Обязательные поля kind-а из реестра отсутствуют в артефакте → `FAIL` с перечнем недостающих полей, handler не запускается.
5. **Handler.** Запустить по `handler_type` из реестра: `rubric` → sub-agent с промптом из `handler_id` (напр. `ontological` для `Episteme`); `deterministic` → детерминированная проверка по `handler_id` (напр. `rule-engine.sh::check-trace-satisfaction` для `WorkDone`); `skill` → вызвать скилл из `handler_id` (напр. `/vdv audit` для `MethodDescription`); `manual` (не должно случиться — такие kind-ы `gate_ready: false`, отсекаются шагом 3) → `NOT_VERIFIABLE`.
6. **`required_lenses`.** Если непусто — запустить ПОСЛЕ основного handler-а, агрегировать по `aggregation_policy` (сейчас единственное значение — `any_required_fail_blocks`: любой обязательный `FAIL` блокирует итог; `CONDITIONAL` у handler-а или линзы НИКОГДА не повышается общей агрегацией до `PASS` — итог = наихудший из вердиктов участников по шкале `FAIL > CONDITIONAL > PASS`, `NOT_VERIFIABLE` не смешивается с этой шкалой и не понижает `PASS`/`CONDITIONAL` соседей, но сам не может быть перекрыт ими).

Verdict этого маршрута — `PASS | CONDITIONAL | FAIL | NOT_VERIFIABLE`, показывается пользователю по формату Шага 3 (поле «Контекст» = `typed:<kind_id>`).
...
- **FAIL:** ≥1 критических
- **NOT_VERIFIABLE** (только типизированный маршрут Шага 0б): проверка технически не выполнена — неизвестный `actual_result_kind`, `gate_ready: false` у известного, либо отсутствует `actual`. НЕ синоним `FAIL`: артефакт не провалил проверку, проверка не операционализирована или не запущена.
