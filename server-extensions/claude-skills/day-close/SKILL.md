...
```
Показать пилоту эти строки **дословно** сразу после свода коммитов (по направлениям, вне направлений, отношение к ограничению недели, даты) — рефлексия идёт по направлениям. Генератор упал → показать строку «не пересчиталось…», НЕ цитировать вчерашний файл как сегодняшний; пустой grep при успешном прогоне → «направления недели в плане не заданы».
...
**5h. WP Context Freshness (БЛОКИРУЮЩЕЕ).** [[gate]] `bash ~/IWE/scripts/day-close-step-log.sh start 2g`. 5a-5c обновляют трекеры (WeekPlan/DayPlan/REGISTRY) — статус-строку, не сам контекст-файл РП. Для каждого РП, тронутого хотя бы одной сессией сегодня (`grep "$(date +%Y-%m-%d)" ~/IWE/MC-sessions/00-index.md` (WP-526 Ф2) + одиночные Quick Close сессии за день): открыть `inbox/WP-N/WP-N.md` **точечно** (grep по номеру/дате сегодняшней сессии → Read с `offset` вокруг найденной строки, не весь файл целиком — для зонтичных РП полное перечитывание дорого, WP-484 peer-session 2026-07-18-13 находка), свериться с §4/§6 отчётов всех сегодняшних сессий по этому РП — суб-пункты, которые сессия закрыла или нашла уже закрытыми, должны быть отмечены done в контекст-файле, а не только в статус-строке трекера. Несколько сессий трогали один зонтичный РП за день → одно согласованное состояние на конец дня, не разрозненные пере-перезаписи. Source: [2026-07-09-17-close-actualization-gap](../../../MC-sessions/2026-07/2026-07-09-17-close-actualization-gap/report.md).
  ...
  **Guarded write (WP-530 Ф5 п.1, 17.08 peer-session с Kimi).** Отметку суб-пунктов done делать НЕ через Edit tool напрямую — через `day-close-5g-apply.sh`, иначе параллельная сессия, тронувшая тот же контекст-файл между Read и записью, теряет свою правку молча (живой прецедент — Ф2 самой карточки WP-530, 15.08):
  ...
  `bash ~/IWE/scripts/day-close-step-log.sh end 2g`
...
4. **Sanity check (БЛОКИРУЮЩЕЕ — реально пересчитать в ЭТОМ ЖЕ проходе, не отложить).** Если получившийся мультипликатор <1.5x при дне с ≥10 peer-сессий, ИЛИ бюджет посчитан только по одному приблизительному методу (например голый `turns_count` без сверки с содержанием сессий) — это не готовый результат, это сигнал недосчёта. Пройтись по каждой крупной сессии (много ходов, несколько коммитов/деплоев, cold-review) и оценить бюджет по факту содержания её карточки/report.md, не только по числу ходов диалога — turns_count систематически занижает сессии с большим объёмом работы внутри одного хода (правки нескольких файлов, деплой на несколько машин, живые прогоны). Записать в DayPlan оба метода (быстрый по числу ходов + уточнённый по содержанию) для сравнения, но основной итог — уточнённый. «Пилота уже нет на связи» не освобождает от пересчёта: это доводится агентом в одиночку в Части Б, ровно как остальные шаги Дня Б. Урок: `lessons_multiplier_peer_sessions_uncounted.md`.
...
**з) Направления недели (РП-561 Ф15):** строки `📅`/`📊`/`⚠️`/`🗓️` из `current/WeekState W{N}.md` — дословно (тот же `grep`, что в Части А, шаг 1, после перегона 5g). Строк `⚠️`/`🗓️` нет → «предупреждений по направлениям нет». Эти строки — единственная часть итогов, которую 14a проверяет побуквенно.

### 14. Запись итогов

`bash ~/IWE/scripts/day-close-step-log.sh start 9a`

**14a.** [[gate]] Дописать секцию «Итоги дня» в DayPlan (шаблон — см. `memory/templates-dayplan.md § Шаблон итогов дня`), включая строку «Направления недели» из 13з.
...
**Валидация «Направления недели» (РП-561 Ф15):** `--verify-dayplan` ниже отдаёт `14a FAIL: WeekState warning not quoted in DayPlan: …`, если хотя бы одна `⚠️`/`🗓️`-строка WeekState с `generated_for` = сегодня не встречается в архивном DayPlan дословно — вернуться к 13з и вставить строку как есть. `14a FAIL: WeekState not generated for …` — в плане недели есть «Направления недели», а генератор сегодня не запускался (шаг 1 Части А или 5g) — запустить его и повторить 13з.
...
- [ ] **WP Context Freshness (шаг 5h):** для каждого РП с сессией сегодня — `inbox/WP-N/WP-N.md` сверен с §4/§6 сегодняшних отчётов, суб-пункты done отмечены в контекст-файле (не только в трекерах)
