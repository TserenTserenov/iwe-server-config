...
**Напоминание — не жди только явного `/archgate` (пилот, 19.08.2026).** Правило уже есть в `CLAUDE.md §5 Pre-action Gates` («Архитектурное решение → АрхГейт `/archgate`») — этот блок не новое правило, а усиление того же напоминания прямо в теле скилла: заметил в разговоре признаки архитектурного решения (новый компонент, интеграция, замена механизма, системное изменение) — предложи или сам инициируй оценку по этому алгоритму, не жди, пока пользователь наберёт `/archgate`.
...
| Ось | Высокая, если |
|-----|---------------|
| **Стоимость изменения** (cost-to-change) | правка затрагивает ≥3 единиц самостоятельной доставки (репозиторий, деплоящийся сервис, платформенный скилл/скрипт), ИЛИ меняет контракт данных/API, ИЛИ защищённые файлы платформы (перечень — `docs/critical-files-map.yaml` шаблона; в авторской копии IWE файл лежит в `FMT-exocortex-template/docs/critical-files-map.yaml`) |
...
Сверь решение с принципами 2-го уровня (DP.ARCH.001 §7) — доменные, из Pack. Затем сверь с принципами 1-го уровня (FPF) — вызови `/fpf` в режиме review (артефакт для проверки — предложенное решение) по полному тексту `FPF-Spec.md` (не по короткой памяти `memory/fpf-reference.md` — она выжимка для перевода терминов, не замена поиску), а не только по двум паттернам из чеклиста современности ниже (§С п.4 — это известные примеры, не потолок). Если решение нарушает принцип любого уровня — сообщи и предложи исправление до подачи на оценку.
...
**Вопрос 2 (A.19.CPM Unified Comparison Mechanism — запрет скрытой скаляризации, опора A.18; в словаре IWE `memory/fpf-reference.md` — «Lawful Comparison»):** «Какие альтернативы рассматривались?»

**Перед тем как задать этот вопрос — самопротиворечие (найдено 04.09.2026, WP-548 Ф4, живой случай WP-562).** Список альтернатив, собранный уже ПОСЛЕ того как решение выбрано, обычно обосновывает выбор задним числом, а не проверяет его — формальность «≥2 варианта» выполняется, даже если ни один вариант всерьёз не спорил с уже принятым решением. Поэтому, прежде чем показать список пилоту: (1) выпиши свой черновой список вариантов — то решение, которое уже выбрано, плюс варианты, которые ты отклонил по пути к нему; (2) сгенерируй ЕЩЁ ОДИН вариант сверх этого черновика — не ради количества, а специально с целью найти изъян в уже выбранном (та же роль, что «правило критика» напарника в пир-сессии DP.SC.154, только применённая к себе). Два исхода, оба фиксируются в вопросе пилоту:
- Новый вариант меняет или уточняет выбор → он идёт в список альтернатив как есть, отмечен что найден на втором проходе.
- Новый вариант ничего не меняет → явно написать пилоту «второй проход (специально против своего выбора) ничего не изменил» — это рост уверенности, не пропущенный шаг.

- Ответ пилота: ≥2 варианта → сравнительная таблица (шаг 2б).
- Ответ пилота: «нет» / «только этот» → продолжай оценку, но отметь в вердикте (шаг 5): *«Оценка без сравнения — уверенность ниже.»*
...
Source-of-truth: `~/IWE/DS-ecosystem-development/C.IT-Platform/C2.IT-Platform/C2.2.Architecture/Data-Governance/B7.3.1-l2-data-classification-map.md` (если файл не найден — ищи в своём DS-ecosystem-development-репо). Если затрагивается чувствительный класс — пройти 6 пунктов:
...
4. **FPF, полный поиск** (WP-525 Ф2, 19.08.2026): Шаг 0 уже вызвал `/fpf` по всему `FPF-Spec.md` — здесь только два паттерна, которые чаще всего оказываются релевантны системным решениям IWE и стоит проверить в первую очередь, а не единственный источник: A.22.CGUS — не путаем декларативную структуру ограничений с линейным порядком исполнения (гейт ≠ порядок мантры, различение в distinctions); E.11.PUA — для разговорного масштаба не тащить сквозную формализацию (проверить релевантность: решение про диалог с пользователем или про машинный конвейер?)
...
- Хотя бы одно правило сработало → остановить дальнейшую оценку ядра.
  ...
  Extensions `checks` (Шаг 4.7). Материал, уже наработанный к этому моменту
  (ветки NBR, черновик DRR), допустимо приложить как справочный — явно вне
  вердикта, не как его часть (стенд 05.09.2026, WP-548). Не обходи `checks` прямым переходом к Шагу 5.
...
**Учёт вердикта (WP-427 Ф8.14, soft-instrumentation).** Если Учётчик следов (trace-accountant) доступен — зафиксировать сам факт вынесения вердикта (не его содержание) вызовом MCP-инструмента `trace`: `sensor_id=archgate`, `event_type=decision_made`, `content={verdict: "проходит"|"не проходит", characteristics_failed: N}`, `prov={actor_type: agent}`. Best-effort — недоступность MCP не блокирует выдачу вердикта пилоту, ошибку не показывать пилоту, просто пропустить молча. **Известное ограничение:** это инструкция агенту, не код — при пропуске (агент забыл/torn context) запись не появится, тайл нагрузки (РП417) останется нижней границей. Критерий отказа механизма (согласовано пир-сессией Kimi+Claude 2026-09-04): если через 30 дней после деплоя ни разу не появилась запись с `sensor_id=archgate` — признать soft-instrumentation нерабочей, искать детерминированную точку.
