...
Путь резолвится через env, дефолт — авторский SoT РП404:

```bash
BUNDLE="${IWE_LOCAL_LLM_DIR:-$HOME/IWE/DS-my-strategy/inbox/WP-404}"
```
...
> **Промоция (триггер «передаём другим», после недели тестирования автором):** при переносе скилла в `FMT/.claude/skills/` бандл финализирует постоянный дом в `extensions/local-llm/`, и дефолт `IWE_LOCAL_LLM_DIR` обновляется на путь установки пользователя. До тех пор скилл указывает на dev-SoT.
