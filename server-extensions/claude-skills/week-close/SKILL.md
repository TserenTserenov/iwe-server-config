...
### 1. Ретро недели, свободный текст (WP-484, решение пилота 30.07 — рефлексия обязательна на всех тактах, CONCEPT-night-cycle.md §22) [[gate]]
...
### 2. Carry-over → W+1, подтверждение пилота [[gate]]

Показать пилоту список РП с pending/in_progress статусом (из WeekPlan текущей недели) — коротко, номер+название, без деталей. Спросить: «Переносим все как есть в новую неделю, или что-то меняется (закрыть/понизить приоритет/добавить)?» Пилот подтверждает или правит список. Сам перенос в новый WeekPlan W{N+1} — механика Части Б (session-prep Пн 04:00 либо ручной прогон), здесь только решение ЧТО переносить.

### 3. R-вопросник (`memory/r-questionnaire.md`, [[gate]])

> **WP-484 Ф4c.** Раньше существовал только в чек-листе внизу файла (не в нумерованном алгоритме) — реальный Close его пропускал, пока `check-trace-satisfaction` не блокировал финализацию постфактум на шаге 17. Теперь явный шаг здесь.
...
Ответы → раздел «R-ответы» в WeekReport (запись — механика Части Б). «Нечего ответить» — валидный ответ (3 недели подряд → вопрос кандидат на удаление, решение пилота).

`bash .claude/hooks/rule-engine.sh mark-gate week-close-g3` (WP-484 Ф4d — трассировка по ходу, не постфактум на шаге 17).

### 4. Оценка качества недели (WP-310 Gap-А) [[gate]]
...
Ответ N → включить `q:N` в commit message шага 16 Части Б.
Если пользователь пропускает → commit без `q:`.

### 5. Мост неделя→месяц: `week_summary` (WP-484 Нить 4) [[narrative]]
...
")
SUMMARY_JSON="{\"effective_week\": \"$WEEK\", \"effective_months\": $EFFECTIVE_MONTHS, \"what_worked\": <ответ шага 1, экранированный>, \"what_didnt\": <ответ шага 1, экранированный>, \"carry_over\": <решение шага 2, экранированное>, \"r_verdict\": <3 ответа шага 3, экранированные как массив>}"
...
### 8. Headless week-review (если включён launchd Пн 00:00)
...
### 9. Статусы РП (closed/partial/not_started/blocked)

**9a.** Закрытые РП: что сделано, ключевые артефакты, мультипликатор за неделю.
**9b.** Частичные: % выполнения, что осталось, перенос в W+1.
**9c.** Не стартовавшие: причина, перенос или закрытие.
**9d.** Заблокированные: блокер, ETA снятия.
**9e.** Перенос в новый WeekPlan W{N+1} — по решению пилота из шага 2 Части А (создаст session-prep автоматически в Пн 04:00 либо вручную).
...
- **Калибровка гипотез (WP-496, из шага 12a):** сверено N записей за неделю; из них с уверенностью ≥80% — доля подтвердившихся (порог: ≥0.9 от заявленной уверенности, иначе «уверенность завышена» — явно отметить в WeekReport). Если сверок не было на этой неделе — не считать, пропустить строку.

### 11. Pending фазы внутри активных РП (B-005)

> **Зачем:** carry-over §2 Части А работает на уровне РП (status: in_progress → перенос). Pending **фазы** внутри Ф-таблиц context-файлов могут потеряться: если родительский РП в `in_progress` — pending-фаза не выделяется автоматически; если родительский ушёл в `done` — фаза теряется вместе с context-файлом. Пилот уже свободен — вместо живого решения (a/b/c) агент **логирует кандидатов** в WeekReport секцией «Pending-фазы — требует внимания», пилот разбирает при следующем открытии.
...
### 12a. Сверка журнала гипотез (LPF, WP-496)
...
### 14. Запись итогов в WeekReport (split, ОПТ-5)
...
2. Дополни секцию «Итоги W{N}» (структура — см. `roles/strategist/prompts/week-review.md`), включая R-ответы (шаг 3 Части А) и оценку качества `q:N` (шаг 4 Части А).
...
### 16. Закоммитить governance-репо
...
# Файлы, изменённые в шагах 7-15 (в массив для pathspec):
...
- [ ] Статусы РП: closed/partial/not_started/blocked разобраны (шаг 9)
- [ ] Ретро недели свободным текстом задано (шаг 1, Часть А, WP-484): оба вопроса заданы пилоту, ответ записан в ledger через `pilot_answer/preclose_retro`
- [ ] **Carry-over подтверждён пилотом (шаг 2, Часть А)**, **мост `week_summary` записан (шаг 5, Часть А)** и **«Ты свободен» сказано (шаг 6, Часть А)**
- [ ] Метрики посчитаны (completion rate, мультипликатор)
- [ ] Carry-over → W+1 выполнен по решению пилота (шаг 9e)
...
- [ ] R-вопросник (3 вопроса, шаг 3 Части А) — ответы записаны в WeekReport
...
- [ ] Оценка качества недели q:N задана (1-5, шаг 4 Части А) и включена в commit message
