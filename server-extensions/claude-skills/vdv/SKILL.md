...
### Шаг 3.1. Записать факт М8 (WP-522 §М, пир-сессия 2026-08-31-26)

Если аудит выше реально прогнан до конца (не прерван пользователем на середине) —
вызвать писателя факта М8 чек-листа участника, best-effort, не блокируя вывод аудита
пользователю (`|| true`, ошибка сети/писателя не должна попасть в ответ):
...
диалога (Шаг 1 build / Шаг 1 audit) — читать оттуда, не выводить заново из того, какие
шаги фактически выполнялись сейчас: `/vdv build` делегирует сюда через Шаг 5, но факт
всё равно `mode: build` — «применил через сборку процесса» и «применил через проверку
готового описания» (`/vdv audit`) для чек-листа разные события. `--evidence` — название
процесса из Шага 1 (build) / Шага 2 (audit), не придумывать заново.

Ответ пользователю не показывать (best-effort), но ошибка НЕ должна пропадать бесследно
(код-ревью 31.08, Critical: `/dev/null` для stderr означало, что при длинном `--evidence`
(>200 символов) или недоступном event-gateway факт терялся молча без единого следа) —
stderr уходит в файл-журнал, не в `/dev/null`. `post-culture-fact.py` сам усекает
`evidence`, если он длиннее лимита схемы (200 символов), с предупреждением в тот же
журнал — событие всё равно уйдёт, просто с усечённым текстом.
