...
- `invalid` + причина → reject этого кандидата, записать в feedback-log, продолжить следующий кандидат.
  После успешной записи подтверждённого отклонения выполнить местный хук М9
  из пользовательской секции, как в Шаге 5в.
...
После успешной записи отклонения выполнить местный хук М9 из пользовательской
секции отдельно для каждого кандидата с `decision_source: pilot`.
...
После успешного коммита выполнить местный хук М9 из пользовательской секции
отдельно для каждого кандидата с `decision_source: pilot` и решением `accept`
или `reject`; для `defer` хук не вызывать. Запись М9 сама по себе не подтверждает
публикацию: её доказательство проверяется в Шаге 6.
...
<!-- USER-SPACE -->
### Местная интеграция М9 (WP-522 §М)

Записывать факт применения процесса после успешной записи отклонения в
feedback-log (Шаг 5в, в том числе ранний выход из Шагов 4в/4г) или успешного
коммита принятого/отклонённого кандидата (Шаг 5г). Требуется подтверждённое
решение `decision_source: pilot`; `defer` и перенумерование предварительного ID
в Шаге 4б не являются отклонением и не создают этот факт.

Вызов выполняется отдельно для каждого кандидата. Повтор сохраняет тот же
`--evidence "<id> из <filename>"`: писатель формирует прежний детерминированный
ключ повтора для event-gateway.
Ошибка дополнительной записи попадает в журнал и не отменяет основную операцию.
