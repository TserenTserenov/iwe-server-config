...
**Сверка с FPF (WP-525 Ф2, 19.08.2026).** Различение «домен ≠ тема» выше — прикладной пересказ первопринципов FPF (A.1 Boundary — граница практики; A.7 Strict Distinction — форма самого различения). Перед финализацией имени (Шаг 2) вызови `/fpf` в режиме review по этим двум паттернам (артефакт для проверки — предложенные домен и различение): сверься, что граница домена и формулировка различения выдерживают полный тест FPF, а не только 3-вопросный чек-лист выше — паспорт Pack претендует на статус первоисточника пользователя, поэтому его форма обязана быть строже, чем у обычного рабочего решения.
...
├── 07-map/                        ← карта домена
└── 09-name-cards/                 ← карточки имён публичных терминов (SPF.SPEC.001 §NameCard, WP-474 Ф8.6c), пустая при scaffold — наполняется по решению автора при первом публичном имени
```
...
`.pfad-decision.md` — PFAD decision record (SPF `E.4.PFAD`), создаётся вместе со scaffold на Шаге 4, наполняется по итогам Шага 2 (и Шага 3, если обсуждалась граница). До WP-474 Ф8.4 файл покрывал только именование (4 таблицы отклонённых вариантов); с Ф8.4 структура покрывает все поля `PrincipleFrameworkArchitectureDecision@Context` (`E.4.PFAD:4`, дословно из `FPF/FPF-Spec.md` — не пересказ), но **не все поля заполняются на Шаге 4**: у только что созданного Pack ещё нет ни паттернов, ни оценки качества, ни публикации — соответствующие поля остаются пустой строкой с пометкой рубежа, когда они заполнятся, это не пропуск.
...
**decisionQuestion:** Какая структура принципов нужна для домена «{предложенный домен}» — отдельный Pack, расширение существующего, или домен не тянет на Pack?

## Зависимости от изданий

**frameworkEditionRef:** этот Pack, `created: {YYYY-MM-DD}`
**fpfCoreEditionRef:** `~/IWE/FPF/FPF-Spec.md` на дату создания (зафиксировать git-SHA, если репозиторий версионируется отдельно)
...
## Источники (sourceBasisRefs, sotaSynthesisPackRefs)

Ссылки на `06-sota/{slug}-sota-sheet.md` и любые внешние источники (статьи, семинары, транскрипты), обосновывающие, что домен вообще существует как отдельное знание. Дозаполняется по ходу Шага 1.5 — не обязано быть полным на Шаге 4.

- ...

## Паттерны и структура (selectedPatternSetRefs, selectedPatternRelationRefs)

**Пусто на Шаге 4 — Pack только что создан, методов ещё нет.** Заполняется по факту: первые записи в `03-methods/` дают первые ссылки сюда. Не рубеж, требующий закрытия перед `Maturity: seed` — прогрессивное поле.

- ...

## Публикация (publicationUnitRefs, accessCarrierRefs, publicationCarrierRefs)

**publicationUnitRefs:** `PACK-{pack_id_slug}` (репозиторий целиком — единица публикации по умолчанию для Pack)
**accessCarrierRefs:** GitHub-репозиторий, если создан на Шаге 4 (`{owner}/PACK-{pack_id_slug}`) — иначе пусто, заполнить при создании
**publicationCarrierRefs:** пусто, если Pack приватный/личный; заполнить, если планируется публичная витрина (см. паттерн WP-401)

## Зависимости от других Pack (dependencyAndEditionRefs)

Какие другие Pack этот использует как основание (не просто «похожая тема» — реальная ссылка `pack_refs` в различениях/методах). Обычно пусто на Шаге 4, растёт по мере written различений.

- ...

## Оценка качества (qualityEvaluationRefs, admissionReviewRefs)

**qualityEvaluationRefs:** пусто до первого запуска `/verify ontological` или package-adequacy проверки для этого Pack.
**admissionReviewRefs:** пусто, пока Pack не выходит на внешнюю аудиторию (публикация, приёмка сторонним ревьюером) — заполнить при первом таком событии, не раньше.

## Последствия (consequences)

Что реально меняется от факта существования этого Pack — не список возможностей, а конкретный эффект (какой домен знания теперь имеет дом, какой РП/процесс теперь на него ссылается).

...

## Условия пересмотра (sourceReturnConditions, refreshOrSupersessionConditions)

**sourceReturnConditions:** при каком событии стоит перечитать источники заново (пример по умолчанию: «если появится новая версия источника из раздела Источники выше» — уточнить конкретнее, если применимо).
**refreshOrSupersessionConditions:** при каком событии само решение о существовании/границе/имени этого Pack нужно пересмотреть (пример по умолчанию: «если появится Pack на смежный домен и границы начнут пересекаться»).
...
**Граница:** ...
**Kind:** ...
**rationaleRefs:** почему выбран именно этот вариант, не только почему отклонены остальные (1-2 предложения, ссылка на ход обсуждения если был)
**decided_by:** pilot
**proposed_by:** agent | pilot
```
Заполнять таблицы вариантами, которые реально обсуждались на Шаге 2 (и Шаге 3) — не изобретать отклонённые варианты задним числом, если пользователь сразу согласился с первым предложением, таблицы остаются пустыми. Секция «Финальный выбор» заполняется на финализации (Шаг 2 «Финализация имени»), не на Шаге 4. Прогрессивные секции (Паттерны и структура / Публикация / Зависимости от других Pack / Оценка качества) не блокируют финализацию имени и не блокируют снятие `Maturity: seed` — они растут вместе с Pack, проверяются отдельно (кандидат: `/verify pack` координата, не заведена в рамках Ф8.4).
...
## Шаг 4.5. Регистрация в реестрах IWE (WP-476 Ф5, 19.07)

> **Источник находки:** до этого шага онбординг нового Pack заканчивался на `git init`/GitHub-репо (Шаг 4) — три места регистрации оставались ручными и необязательными, каждое обнаруживалось отдельно (`PACK-systems-art` не был подключён к `sources.json` до WP-476 Ф5, `docs/repo-inventory.md` («Каталог Pack-репозиториев») официально отставший в `registry-catalog.md`). Ротация `knowledge-audit-watchdog.sh` (DP.SC.061) регистрации не требует — она читает `PACK-*` прямо с диска (`find ~/IWE -maxdepth 1 -name "PACK-*"`), новый Pack подхватывается автоматически при следующем ежемесячном запуске.

Выполнить оба пункта сразу после Шага 4 (scaffold + git init), не откладывать на «потом»:

1. **`DS-MCP/knowledge-mcp/scripts/sources.json`** — добавить запись `{"source": "PACK-{slug}", "source_type": "pack", "path": "~/IWE/PACK-{slug}/pack", "exclude": ["images/", "assets/"]}`. Путь `path` зависит от структуры: если у Pack есть вложенная директория `pack/` (стандарт, см. Шаг 4 scaffold) — путь `~/IWE/PACK-{slug}/pack`; если файлы лежат прямо в корне репо (нестандартная структура, прецедент — `PACK-agent-rules`) — путь `~/IWE/PACK-{slug}` без вложенной директории. Без этой записи семантический поиск (`knowledge_search`) не увидит содержимое нового Pack — пробел обнаруживается только случайно, когда кто-то ищет и не находит.
2. **`DS-my-strategy/docs/repo-inventory.md`** («Каталог Pack-репозиториев») — добавить строку `| PACK-{slug} | Pack: {одна строка описания домена} |` в секцию `<summary><b>Шаблоны и базы знаний (FMT-* / PACK-*)</b></summary>`. Если Pack ещё не привязан к GitHub (Шаг 4 «Опционально») — пометить явно `(ещё не привязан к GitHub)` по прецеденту `PACK-systems-art`, не молчать об этом.

Оба пункта — правка одной строки в уже существующем файле, не создание нового процесса; коммитить вместе с Шагом 4 (`git add -A` в Шаге 4 их не подхватит — оба файла в других репозиториях).
