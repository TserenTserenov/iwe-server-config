...
2. Найти контекст:
   ...
   - `subject = team` → `~/IWE/DS-my-strategy/team-guide/`
   - `subject = organization` → `~/IWE/DS-my-strategy/org-guide/`
...
Загрузить `~/IWE/PACK-MIM/pack/mim/03-methods/MIM.M.031-rr-guide-routing.md` (Read tool).
...
### Аналитик ограничений (DP.ROLE.054, `/bottleneck-pick`) vs R31 — пробел в описании роли

Пробел в роли (у работы нет владельца) часто предшествует узкому месту, но не равен ему. Критерий развилки (WP-481 Ф7 п.6, пир-сессия с Codex 11.08.2026): **эффект на поток подтверждён** (пробел уже держит конкретную работу в очереди или срывает срок) → к Аналитику ограничений; **не подтверждён** → сначала сюда, к R31: это вопрос владения на уровне устройства организации, без немедленной остановки потока. Тот же критерий записан в триггерах `/bottleneck-pick`.
