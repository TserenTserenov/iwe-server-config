...
**Дальше:** [из WP-context «Следующий шаг», нормализовано в форму «кто действует + нужен ли ответ пилота»] — примеры: «агент продолжит с <шаг>, от вас ничего не требуется» / «нужна ваша развилка: A или B» / «работа завершена, от вас ничего не требуется» (нет открытых пунктов)
```

> **Поле «Дальше» обязательно (A12, БЛОКИРУЮЩЕЕ) и всегда последнее** — рендерится после `Фон`, а не перед ним, даже когда `Фон` не выводится (status ≠ in_progress_background). Источник данных — тот же `Следующий шаг` из «Осталось» (см. выше), но не копия один в один: `Следующий шаг` называет только следующий объект работы, `Дальше` обязано явно назвать субъекта действия (агент/пилот) и есть ли ожидание ответа. Нет открытых пунктов → «работа завершена, от вас ничего не требуется», не пустая строка.
