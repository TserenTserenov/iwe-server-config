...
3.5. **Мини-ритуал переоткрытия (РП-541 Ф5, S-59, 20.08.2026; источник даты — детерминированный скрипт, не ручное поле карточки, WP-7 пир-сессия 2026-09-10-13, Claude+Kimi+Codex, 10.09.2026).** `last_session` в frontmatter карточки — никем механически не поддерживается (вписывается вручную, у большинства карточек отсутствует вовсе, WP-170 — живой пример) и с этого дня БОЛЬШЕ НЕ ИСТОЧНИК для этой проверки; кто ведёт там нарративную заметку (как РП-484) — может продолжать, скрипт её не трогает и не читает. Вместо парсинга поля — вызов:
    ```bash
    python3 "${IWE_GOVERNANCE_REPO_PATH:-$HOME/IWE/DS-my-strategy}/scripts/wp-last-session-days.py" WP-N
    ```
    Вывод одной строкой, парсить без LLM-догадок: `freshness=<fresh|stale|unknown> days_since=<N|NA> source=<ledger|ledger-window-exhausted|git-log-approximate|none>` (источник — day-ledger `session_closed`/`session_closed_direct` события за последние 14 дней; git log карточки — только запасной путь, если сам ledger физически недоступен, не если он прочитан и пуст). **Это «сессионная свежесть» (когда последний раз здесь работали) — отдельная ось от «Git-синхронизация рабочей копии» (GIT_SYNC_STATUS из Шага 3a, отстаёт ли копия от origin); одно не подменяет другое, обе показывать пилоту раздельно, не сводить в одно слово «свежесть».** `freshness=fresh` → пропустить молча. `freshness=stale` ИЛИ `freshness=unknown` (неизвестность не равна «пропустить» — безопасный дефолт) → после Sync Gate показать пилоту короткую сводку и дождаться согласования ДО начала работы: (а) что изменилось вокруг РП со времени последней сессии (из bundle: связанные РП, drift-сигналы; bundle недоступен — race-guard пропустил 3a или файл уже удалён 3d → запустить `wp-sync-bundle.sh WP-N --force-sync` заново только для сводки — с флагом, иначе повторный `BUNDLE_RC=3` останавливает и эту сводку тоже), (б) открытые фазы, (в) предложение «что делаю в этой сессии». В сводке — две раздельные строки: «Сессионная свежесть (ledger): …» и «Git-синхронизация рабочей копии: …» (из GIT_SYNC_STATUS/GIT_SYNC_DETAIL). Полный Ритуал не требуется — только сводка + «да» пилота. [[gate]]
