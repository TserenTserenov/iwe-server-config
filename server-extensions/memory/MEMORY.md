...
| 149 | 🔄 | **P1** | Персональные руководства | 31.07 поздн. веч.: 2-й раунд ревью (Fable 5) подтвердил все 7 фиксов 1-го раунда, +5 мелких находок. Живая приёмка на цехе-1 (тестовая копия кода, реальный GitHub+LLM, `--probe`, публикации не было) нашла и починила критичный баг ВНЕ плана Р1-Р8а: `get_file_content` строил URL без экранирования пути — имя файла плана недели со скобками/пробелами (стандартный формат) давало 404, «Приоритетные РП недели»/«Фокус недели» молчали НИКОГДА, юнит-тесты не ловили (мокали на уровень выше). Исправлено (`_content_api_url`), подтверждено вживую: табло реально собрало «Рабочая повестка» (WP-503/149/484/406/458). Итог дневного рендера сегодня — жёлтый (0❌/3🟡, все содержательные, не кодовые), НЕ зелёный — условие пилота «зелёное→заливай» не выполнено буквально, решение за пилотом. Тесты 624 прошли (+3), nix-check чисто, сервер прибран (git status чист). Ничего не закоммичено. Детали `inbox/WP-149/чеклист-работ-до-зелёного-рендера.md` |
...
| 484 | 🔄 | **P1** | Автогенератор открытия/закрытия дня (зонтичный) | 31.07 утро: Ф31 выполнена — конвейер дня целиком на цех-1, открытие 31.07 прошло живьём (21/21), Мак чист от 9 launchd-заданий цикла, затор цеха (100 коммитов) разобран. Пилот поправил модель месяца: ночной `month-open-orchestrator.sh` (23:0x сегодня, резерв 02:15 завтра) — НЕ закрытие месяца, только механический ledger-rollup + черновик MonthOpen.md; настоящий `/month-close` (Strategy.md, ретро, фаза/калибр) пилот запускает сам живым разговором в любой момент, автоматика ничего не блокирует и не заменяет. Ф32 (доработки, 11 пунктов) явным решением пилота отложена на завтра 01.08, сегодня не начинать; приоритет внутри неё — п.6 неделя (сломана, выключена) до Вс 02.08. 33 фоновых Мак-задания вне цикла — вынесены отдельной фазой в WP-7. Next: утром 01.08 сверить контрольные точки ночи (список в Ф31 карточки), затем Ф32. |
