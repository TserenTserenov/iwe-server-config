...
| 391 | 🔄 | **P1** | Браузерный IWE — мультимодельный вход (Kimi) | 28.07 веч.: Ф7.3 (адаптер) задеплоена, живой прогон PASS; Ф7.4 (per-user OAuth вместо single-token, АрхГейт пройден) — 5/8 пунктов кодом, запушено в kimi-adapter+DS-my-strategy+FMT; блокирует WP-385 Ф5 — next Ф7.4.6 живой прогон multi-user (нужен пилот: Railway env+миграция+2 аккаунта) |
