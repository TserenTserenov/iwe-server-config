...
> **РП484 (зонтичный):** Ф1-Ф2 done; 18.07: git-lock сработал в бою впервые (маркер `day-close-start: 2026-07-18` подтверждён на origin/main); окно открытия дня на сервере сдвинуто на 4:30; грязная копия tsekh-1 расчищена. Ф3 — сбор таймингов 3-5 дней продолжается; next: структурный фикс устойчивости pull/guard к грязному индексу. Контекст: `inbox/WP-484/WP-484.md`.
...
| 330 | 🔄 | **P1** | Марафон вторая волна | 18.07 встреча 3 прошла (транскрипт+разбор), но 3 решения снова не донесены → перенос на 22.07; урок роль-метод-продукт дублирует черновик D-048, решение пилота ждёт |
...
| 5 | 🔄 | — | Платформа: развитие (зонтичный) | 19.07: issue #271 (внешний баг-репорт, macOS-пути ломали Docker-раннеры) закрыт — 9 файлов на runtime-резолв (5cd5695); попутно найдены и починены 2 несвязанных CI-блокера (strategist.sh, generate-hot-files-list.sh); независимый субагент подтвердил готовность; версия шаблона поднята 0.35.5→0.36.0 (не поднималась 18 дней), CI зелёный; next: e2e-чек бота /setup, env-риск USER_PROFILE_SERVICE_URL остаются открытыми с 18.07 |
...
| 170 | 🔄 | P1 | База знаний: обновление по разбору информации | очередь пуста (0 pending); 18.07 веч. слияние реестров DP.D решено и реализовано (SPF-доки+гард); next: миграция 119 секций 01B — отдельный РП, не срочно |
...
| 483 | 🔄 | P2 | Комплект структурирования данных и руководств (guide-kit) | 18.07: **релиз v0.1.1 выпущен** — тег на remote, синк в шаблон закоммичен+запушен (версия шаблона 0.35.5, срез всем НЕ делался — отдельное решение Week Close), сервер tsekh-1 подтверждён на фиксе `d81a73c` (авто-подтягивание). **Контекст актуализирован по РП495/493/149:** Ф6-Ф8 → downstream РП495 Ф3-Ф5 + РП493 Ф7 (основания уехали вверх, Ф7 не оценена, РП495 Ф4 ждёт калибровки до ~сентября); Ф10 блокирована каналом Здоровья РП470. **IntegrationGate Ф9 пройден:** реализация упирается в серверный Родник (№8/№16, вне W29); транспорт можно свести к существующему MCP платформы. Проверено независимым субагентом (1 пограничное замечание по SHA — поправлено). next: решение пилота по Ф9 (транспорт + серверная часть в scope?); критический путь направления — концепция РП495, не guide-kit |
| 488 | 🔄 | P2 | Браузерное рабочее место разработчика на Hetzner | Ф1 принят; next: Ф2 скорость |
| 484 | 🔄 | — | Автогенератор открытия/закрытия дня/недели/месяца | 18.07: окно открытия дня на сервере сдвинуто на 4:30 (сужено до 6:59 по вопросу пилота); найден и обойдён новый баг — гейт закрытия дня доверяет отставшей локальной git-истории (грязная копия сервера, 50+ файлов, блокирует pull), план на 18.07 закрыт вручную в изолированном клоне (чек-лист 21/21 после дофикса РП470-раздела); открытие дня отдельно переключено на типовой раннер (другая сессия, первое боевое срабатывание завтра 09:15); грязная копия расчищена (сброс на origin, 66/68 файлов = origin побайтно, бэкап-bundle, с разрешения пилота); pull снова проходит, day-close 17.07 виден локально; субагент подтвердил + нашёл рецидив (17 файлов свежей работы чужих сессий, не потеря); next: структурный фикс - устойчивость pull/guard к грязному индексу (разовая расчистка 23:00 не защищает структурно), приоритет поднят |
...
- [lessons_component_sync_is_not_version_cut.md](lessons_component_sync_is_not_version_cut.md) — синк компонента в шаблон ≠ срез версии всем (утягивает чужой [Unreleased]); авто-пул сервера ≠ ручной синк шаблона (18.07)
...
- [lessons_openrouter_403_is_spend_limit_not_stale_key.md](lessons_openrouter_403_is_spend_limit_not_stale_key.md) — OpenRouter 403 ≠ 401: сначала проверить `/api/v1/key` (лимит трат), не сразу считать ключ устаревшим (guides-mcp, 18-19.07)
- [lessons_git_branches_not_remotes_false_positive_stale_pr.md](lessons_git_branches_not_remotes_false_positive_stale_pr.md) — `git log --branches --not --remotes` врёт про unpushed на старых PR-ветках — проверять по активной ветке (19.07)
