...
|-----|----|----|----------|----------------|
| 498 | 🔄 | P2 | Наставник ИИ: оперативная помощь в чате | Ф1 АрхГейт закрыта — составная роль Диагност⊗Навигатор; next: Ф2 паковая спецификация |
...
| 483 | 🔄 | P2 | Комплект структурирования данных (guide-kit) | 20.07: дрейф закрыт (swimming.yaml восстановлен, gitignore-пробел); адаптер domain_traits→продакшен закрыт, 2 независимые проверки нашли и закрыли 2 находки (guide-kit@f92b8f7, 310/310 тестов); Ф6-Ф8 больше не блокируются РП493 (Ф7+Ф8 РП493 закрыты) — остаётся только РП495 Ф4-Ф5; Ф10 не зависела от РП493, блокер только WP-470 |
...
| 493 | 🔄 | P2 | Лаборатория характеристик | 20.07: Ф8 needs-decision закрыт (тайминг публикации growth-traits-lab подтверждён пилотом — «сейчас», ретроактивно совпал с фактом Ф9 19.07); снят downstream-блокер РП483 Ф6-Ф8 по линии РП493. Открыт новый мелкий пункт: reusable workflow автоперевода + реестр auto-update не реализованы (не блокирует). Ф4 по-прежнему ждёт данных (календарь 31.08) |
| 494 | 🔄 | P2 | Панель рабочих продуктов в VS Code | ручная F5-приёмка пилотом |
| 495 | 🔄 | **P1** | Концепция персонального развития | 20.07: Ф7 прогон запущен — 5-8 человек (марафон+СС/МС) прошли self-service запуск, генерация идёт; фокус смещён на содержание (деградирует честно при малой истории, критерий — день1 vs день7-10 у одного человека); блокер прогона — качество генерации в РП149 сначала (10 пунктов сведены туда) |
...
| 167 | 🔄 | P5 | Публикации (зонтичный) | анонсы 4 направлений draft (#178-181), названия финал; пост #169 ждёт |
