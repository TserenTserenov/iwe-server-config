...
| 149 | 🔄 | **P1** | Персональные руководства | 31.07 пир-сессия с Codex: Р7 (снимок базы знаний) оказался уже задеплоен автоматикой цеха-1 (таймер сам тянет iwe-server-config с GitHub) — чек-лист был устаревшим; отдельно найдено, что DS-autonomous-agents цех-1 НЕ тянет автоматически (в отличие от iwe-server-config) — код был устаревшим на сервере, подтянут вручную. Находка 1.13 (вчерашний файл отсутствует → честное ⬜, было ложное 🟡 через судью) и 1.3/1.15 (машинная проверка покрытия программ по долям + вайринг в retry) закрыты в коде + Р9 гигиена (4 пункта); cold-review нашёл и починил критичный краш ночного батча (нестандартный guide.json ронял всех пилотов разом). Живая проверка (--probe, 3 итерации с пилотом): красный → жёлтый с побеждающим повтором — доминирующая по доле программа исключена из требования отдельного абзаца (правка логики проверки по инсайту из живых данных, не текста промпта). Пилот решил остановиться на жёлтом, ждать настоящего ночного рендера; next: свериться с `checks/2026-08-01.md` утром — точный список находок 3-го пробного прогона не сохранился (SSH оборвался, восстановлен только агрегат из metrics.jsonl). Детали `inbox/WP-149/чеклист-работ-до-зелёного-рендера.md`, сессия `DS-my-strategy/sessions/2026-07/31/2026-07-31-04-wp149-finish-checklist/report.md` |
...
| 484 | 🔄 | **P1** | Автогенератор открытия/закрытия дня (зонтичный) | 31.07 поздн.: Ф34 machine/ledger/ готова. Ф35 — двойной косяк за один день на одном действии: (1) агент исполнял старую модель закрытия сессии, найдены и переписаны 3 корневых файла на DP.D.288 Часть А/Б, независимо проверено субагентом; (2) закрывая ЭТУ ЖЕ сессию, агент не воспользовался только что исправленным механизмом — закрыл по памяти о своей правке, пропустил проактивную рефлексию и финальную сверку. Пилот: «снова неправильно закрыл сессию» → передано в новую сессию (открытые пункты и урок Правило 37 в WP-484.md Ф35). Next: новая сессия — живой прогон раннера + пункты из Ф35, затем утро 01.08 контрольные точки ночи + Ф32. |
...
- [lessons_ssh_drop_recover_from_metrics_file.md](lessons_ssh_drop_recover_from_metrics_file.md) — SSH оборвался посреди живого `--probe` на цехе-1 → сервер всё равно доработал, восстановить результат из `metrics.jsonl`, не перезапускать вслепую
