...
> **РП484 (зонтичный):** Ф1-Ф2 done, гейт-В задеплоен 15.07. Ф3 ждёт боевого 20.07. Контекст: `inbox/WP-484/WP-484.md`.
...
| 471 | 🔄 | P2 | Карта целевых аудиторий экосистемы | iron-man-model §2в (роли, Маск, Вселенная); валидация метафоры стартует постами |
...
| 458 | 🔄 | **P2** | Сквозной аудит безопасности платформы и IWE | Волна 4 закрыта на 7/9 (17.07); осталось 2 архитектурных (I8/ВЫ-9, ВЫ-8) — за пилотом; +ВЫ-13 (GitHub OAuth широкий scope) найдена и передана в WP-406 Ф22 |
...
| 117 | 🔄 | — | Развитие nudge-системы | PR #291 смержен 17.07 (смоук 327/327, деплой ушёл); next: приёмка логов 13:00 + вопрос cp_assessments (пилот) |
| 149 | 🔄 | **P1** | Система генерации персональных руководств | 17.07 пауза снята (Ф1.5 РП483 закрыта 9/9, старая тройка выведена из эксплуатации); заведена Ф-learning-history-bkt (источник данных мастерства физически отсутствует, нужен АрхГейт); next: пилот выбирает приоритет — Ф-learning-history-bkt или ФК5+ФК22 |
...
| 455 | 🔄 | P4 | Неизменяемость аудит-журнала событий | Ф10.1-Ф10.3 закрыты; next: Ф11 анкоринг |
...
| 7 | 🔄 | — | Платформа: техдолг (зонтичный) | 17.07: Issue Funnel 268/267/266 закрыт (пир-сессия+дозакрытие после независимой проверки — разрыв доставки в шаблон найден и устранён, все 3 issue закрыты на GitHub); + разрыв ночной очереди Кими — решение пилота по 4 фиксам раннера |
| 485 | 🔄 | P2 | Сверка дублирующихся скриптов root↔шаблон | 17.07: клон-омоним DS-IT-systems/DS-ai-systems вычищен из репо и из 5/6 живых launchd (6-й, weekreview, уже был на верной `.iwe-runtime`-архитектуре WP-273); root↔сервер (tsekh-1) — отдельная 3-я копия, 117 файлов дрейфа server-extensions, решение пилота; Ф4 done, часть промоции уже задеплоена (artifactor.py/iwe-consent.sh/iwe-w-reflection.sh/day-open-preflight.sh); next: 8 файлов кластера Kimi Standalone + WP-295 (решения пилота) |
...
| 476 | 🔄 | **P1** | Модель жизненного цикла данных (конвейеры 2.1-2.4) | Ф3 закрыта 17.07 (реестр PIPE-01..12 + экспорт сырья WakaTime, решения пилота); next: Ф2 Pack (живой пилот) / Ф5 / Ф6 |
...
| 483 | 🔄 | P2 | Комплект структурирования данных и руководств (guide-kit) | Ф1.5 закрыта 9/9 17.07 (миграция ядра, блокер РП149 снят); Ф4+Ф5 закрыты (Ф5 кроме серверной спеки подписки, блокер WP-467); work_section:iwe включён после Security Gate B7.3 и живой проверки на реальных данных (2 бага найдены и починены); next: Ф6-Ф10 по очереди |
| 488 | 🔄 | P2 | Браузерное рабочее место разработчика на Hetzner | Ф1 принят; next: Ф2 скорость |
| 484 | 🔄 | — | Автогенератор открытия/закрытия дня/недели/месяца | 17.07 вечер: server-primary для Day Close реализован и задеплоен (git-lock, пир-сессия+2 раунда ревью+8 тестов); next: проверить первое боевое срабатывание 23:00 МСК, собрать тайминги 3-5 дней |
...
| 494 | ⏳ | P2 | Панель рабочих продуктов в VS Code | создан 16.07 (DS-iwe-wp-panel); Ф1 scaffold не начат |
...
- [reference_macos_zsh_env_quirks_hub.md](reference_macos_zsh_env_quirks_hub.md) — квирки macOS/zsh/grep/git (11 хабом)
