...
| 484 | 🔄 | **P1** | Автогенератор открытия/закрытия дня (зонтичный) | 11.08 Ф85/Ф86 (пир с Codex): АрхГейт гонки сессий → кандидат E' зафиксирован (не реализован); 11.08 (отдельно): корневой баг открытия дня (regex+set-e/pipefail в day-open.checks.md) найден и починен, tsekh-1 ретрай зелёный; next: измеримый пилот Ф85/86 отдельным заходом; ещё open: живое подтверждение 17.08 |
