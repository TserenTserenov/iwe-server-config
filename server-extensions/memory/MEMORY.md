...
|-----|----|----|----------|----------------|
| 516 | 🔄 | P2 | Контур самоулучшения IWE | Ф1 готова структурно; next Ф2 — расширить поиск кандидатов |
...
| 149 | 🔄 | **P1** | Персональные руководства | Раскат открыт всем (задеплоено); F6 (знания вне pipeline) ждёт приоритета |
...
| 7 | 🔄 | — | Платформа: техдолг (зонтичный) | 10.08: Ф61 (backlog семафоров) + Ф62 (конвейер доставки без барьера перед релизом) открыты; next: решение пилота по приоритету (детали → WP-7.md) |
...
| 481 | 🔄 | **P1** | Каталог применений FPF-семинара к IWE | 3 аудита + Ф23 закрыты; next разбор 5-го семинара (архив найден 10.08) |
...
| 484 | 🔄 | **P1** | Автогенератор открытия/закрытия дня (зонтичный) | 10.08 Ф79+Ф80 закрыты и подтверждены живьём (idempotency, DayPlan-рендер, scope-gate) — но пилот назвал РП484 «узким горлышком» (месяц без автоплана), next его решение по приоритету + разрыв шаблона WeekPlan (Check 10) |
