...
| 429 | 🔄 | — | Детектор непротиворечивости базы | 28.07: 4 хвоста закрыты пир-сессией (б разобран, CC-125 создан, phys-program удалён, слой-2 зафиксирован) — РП готов к закрытию, живой хвост только CC-125 у пилота |
...
| 7 | 🔄 | — | Платформа: техдолг (зонтичный) | 27.07 веч.: гейт запуска закрыл ещё 4 из 5 переменных auth-gateway.py (NEON_*/PERSONA_URL), задеплоен и подтверждён живым тестом; независимая проверка подтвердила выбор границы grep'ом по коду — 3 остальные (ORY_BASE_URL/DATABASE_URL/SUBSCRIPTION_DATABASE_URL) осознанно не гейтить, легальные пустые режимы |
