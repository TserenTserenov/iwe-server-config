...
| 7 | 🔄 | — | Платформа: техдолг (зонтичный) | 04.08: пир-сессия с Codex по всем 15 открытым issue шаблона — закрыто 6 (#343/#338/#339/#340/#331/#348), CI шаблона вернут в зелёное после 5 красных прогонов, заведён сторож неподключённых хуков; за пилотом 3 решения (обёртка #347, гейты #310/#321/#323, 6 хуков разрыва доставки); 03.08 веч.: Ф-MemoryBudget — S-53 введён (контракт записи, git памяти, замер), уборка таблицы ночью; 03.08: Ф-script-contract-gate закрыта (3 раунда фиксов); Ф39 (FMT #344-346) закрыта; +2 фазы сторожей (реестр, семафоры); 04.08: Ф46 (auto-orphan) закрыт находкой — уже решён в РП484 Ф49; 04.08 веч.: Ф44 (пир-адаптеры Codex↔Kimi/Claude) закрыта — 3 независимых аудита, 18/18 тестов, живой артефакт, передача в новую сессию (next: `template-sync.sh --dry-run` для kimi-peer-writer/SKILL.md); детали `inbox/WP-7/WP-7.md`; решения пилота ждут |
