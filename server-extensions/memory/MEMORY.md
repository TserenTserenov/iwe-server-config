...
| 499 | 🔄 | P3 | Механизм подключения и переключения агентов IWE | 28.07 поздний вечер: Ф9b (инструкция подключения агента, вендор-агностично, самодостаточна для агента) + Ф12 (проверка FPF/SPF/Pack в MCP знаний) закрыты по вопросам сообщества — обе доставлены в шаблон, верифицированы чистым clone; next Ф9 разведка штатного механизма VS Code (открыта, не тронута) |
...
| 149 | 🔄 | **P1** | Персональные руководства | 28.07 веч.: единственный красный блокер W31 (1.9 мост к РП в weekly) починен — weekly требует явный код РП, симметрично daily; commit 4d62ae0, 568 тестов (21 pre-existing не мои); next живая приёмка на понедельничном рендере W32 |
...
| 391 | 🔄 | **P1** | Браузерный IWE — мультимодельный вход (Kimi) | 28.07 ночь: Ф7.4+Ф7.5 задеплоены; независимая проверка нашла 8 находок (3 критичных) — все применены и повторно задеплоены (main 33cf90d, 39/39 тестов); блокирует WP-385 Ф5 — next живой прогон реальным ключом Moonshot (нужен пилот) |
