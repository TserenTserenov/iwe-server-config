...
| 510 | 🔄 | P3→P1 | ИИ-личности | 03.08 веч.: Ф10 Гари-онбордер — дом=цех-1 и Kit .2 применены, инструкция цеху ждёт прогона; next: dry-run цеха, Патч 4 (метка сессий) отдельной сессией, генератор Public Kit; DR-03 ждёт приёмки |
...
| 484 | 🔄 | **P1** | Автогенератор открытия/закрытия дня (зонтичный) | +Ф42 РП-7: раннер падает на длинных commit-сообщениях, чинить count_active_cards первым |
