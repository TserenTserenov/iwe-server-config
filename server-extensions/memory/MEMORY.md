...
| 484 | 🔄 | **P1** | Автогенератор открытия/закрытия дня (зонтичный) | 01.08: Ф36 реализована (день+неделя), hermes-баг починен, day-open подтверждён чистым, ночной цикл диагностирован (не починен); отдельно — сверка списка ошибок с Кодексом, 2 фикса (orphan detection, portable path) независимо проверены, структурный пробел найден и смягчён WARN-логом. Next: пилот — приоритет между ночным циклом и root-cause `$IWE_SCRIPTS`. Детали → `inbox/WP-484/WP-484.md` |
