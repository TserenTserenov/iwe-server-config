...
| 149 | 🔄 | **P1** | Персональные руководства | 22.07: пир-сессия с Кими — 5 фиксов рендера задеплоены+3 живых прогона (корень: стратконтракт месяц не читался, путь current/→machine/; запрет глаголов в обеих ветках; section_floors sum==floor; SET LOCAL для Neon-табло; формат-страж после живого регресса H2); 📏 streak=2 алерт подтверждён пилотом; next: 2-3 ночи наблюдения 2.4/2.5, решение пилота по порогу объёма (модель насыщается 1200-1400 при 1474), drift G6/Ученик до снятия паузы |
