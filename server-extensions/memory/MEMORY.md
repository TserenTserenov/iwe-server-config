...
| 484 | 🔄 | — | Автогенератор открытия/закрытия дня (зонтичный) | 29.07: старый порядок убран для дня/недели/месяца (честный отказ вместо fallback везде), месяц впервые подключён к расписанию, автоочистка заметок больше не решает за пилота — next дождаться воскресенья/31.07 ночи/сегодня 22:00 |
