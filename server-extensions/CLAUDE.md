...
> Kimi → `AGENTS.md`, Hermes → Aisystant MCP `get_instructions`; доставка ядра в шаблон — `DS-ai-systems/setup/scripts/template-sync.sh` (защищённый конвейер, `--dry-run`/`--check`). Slim-ядро: триггеры + правила hot; детали → `memory/`, `.claude/rules-lazy/`, `.claude/skills/`.
...
- Замена legacy-компонента ИЛИ первый прод-запуск с записью в непустые боевые таблицы → **LegacyPortGate (БЛОКИРУЮЩЕЕ):** сначала 15-мин субагент «как это работает сейчас?» → `memory/feedback_behaviour.md` П.10.
...
Рабочая директория: `/Users/tserentserenov/IWE/`; `memory/` = симлинк на auto-memory.
...
Hot-каркас ≤20K токенов (M1), строгая цель ≤12K (M2). Изменил файл из `hot-files.list` (оба CLAUDE.md, rules/*.md) → перед коммитом `DS-my-strategy/scripts/verify-context-budget.sh`.
