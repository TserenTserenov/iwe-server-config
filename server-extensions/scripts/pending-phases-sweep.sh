...
#
# Использование:
#   bash pending-phases-sweep.sh                  # обход активных РП в DS-my-strategy/inbox/
    ...
    if ! scan_output=$(
        awk '
            ...
            function is_closed(s) {
                ...
                # "актуальнее выполнено" ложно матчится ("не" как подстрока
                # перед пробелом).
                if (s ~ /✅|\[[xX]\]|[Dd]one|[Cc]ompleted|[Cc]omplete|[Cc]ancelled|[Cc]anceled|[Ss]kipped|[Bb]locked/) return 1
                if (s ~ /(^|[^а-яёА-ЯЁ])(Не|не)(Выполнен|выполнен|Завершен|завершен|Закрыт|закрыт|Отменен|отменен)/) return 0
                if (s ~ /(^|[^а-яёА-ЯЁ])(Не|не)[[:space:]]+(Выполнен|выполнен|Завершен|завершен|Закрыт|закрыт|Отменен|отменен)/) return 0
                return s ~ /(^|[^а-яёА-ЯЁ])(Выполнен|выполнен|Завершен|завершен|Закрыт|закрыт|Отменен|отменен)/
            }
