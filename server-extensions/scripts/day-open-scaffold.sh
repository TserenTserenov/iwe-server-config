...
render_world() {
  ...
  # Ф32 п.1 (WP-484, 31.07): пустой/упавший источник раньше давал непроходимую пару
  # проверок (News Lens требует заполненный Вывод, «Мир без URL» запрещает вывод без
  # ссылок) — LLM «решал» это галлюцинациями. Теперь Вывод остаётся PENDING ТОЛЬКО
  # когда есть реальный список новостей; пустой и упавший источники получают
  # детерминированный Вывод «нет данных» — честно и проходимо для обеих проверок.
  local news_out news_rc
  news_out=$(bash "$IWE/scripts/server-news.sh" "$CONFIG" 2>/dev/null); news_rc=$?
  if [ "$news_rc" -eq 0 ] && [ -n "$(printf '%s' "$news_out" | tr -d '[:space:]')" ]; then
    echo "$news_out"
    echo ""
    echo "**Вывод:** <!-- PENDING: news-lens — 2-4 предложения: какие из этих новостей релевантны активным РП. Использовать контекст WeekPlan + WP-Registry. -->"
  elif [ "$news_rc" -eq 0 ]; then
    echo "> нет новых материалов за сегодня (server-news.sh вернул пустой список)"
    echo ""
    echo "**Вывод:** нет данных — источники не вернули новых материалов."
  else
    echo "<!-- PENDING: world — RSS feeds недоступны (server-news.sh завершился с ошибкой). Каждый пункт = markdown URL. -->"
    echo ""
    echo "> ⚠️ RSS-фиды недоступны — пометить 🔴 в «Требует внимания»."
    echo ""
    echo "**Вывод:** нет данных — RSS-источники недоступны (server-news.sh упал)."
  fi
  echo ""
  echo "</details>"
}
...
render_scout() {
  ...
  if [ -d "$scout_dir" ]; then
    ...
    # WP-7 DOC4: DS-agent-workspace отсутствует на этой машине — Scout не установлен, не сбой.
    echo "> Scout не установлен на этой машине — секция неактуальна"
    echo "> **Статус ревью:** — (не установлен)"
  elif [ "${SCOUT_PF:-unknown}" = "ok" ]; then
    # WP-7 DOC4: preflight уже отличил «нет задач в очереди» (NO_TASKS, штатно) от
    # реального сбоя — раньше здесь показывалось то же самое сообщение об аварии.
    echo "> Нет отчёта на $DATE — этой ночью не было задач в очереди сканирования, это штатно"
    echo "> **Статус ревью:** — (нет находок)"
  elif [ "${SCOUT_PF:-unknown}" = "fail" ]; then
    echo "> Нет отчёта на $DATE — Scout не отчитывался, хотя в очереди есть незакрытые задачи (см. «Требует внимания»)"
    echo "> **Статус ревью:** ⬜ требует внимания"
  else
...
render_yesterday() {
  ...
  # repo that doesn't exist (renamed to DS-my-strategy) and silently makes this `cd` fail,
  # which always empties dc_committed regardless of the grep below (bug 2026-07-02).
  local dc_committed
  dc_committed=$(cd "$IWE/DS-my-strategy" && git log --since="$YDAY 00:00:00" -i \
...
**Портфель в работе:** <!-- PENDING: budget — посчитать после плана, формат см. templates-dayplan.md (переименовано из «Бюджет дня» 2026-09-01, Ф148/WP-484 — портфель РП всего / физ / мультипликатор). -->
