...
#
# Обходит DS-my-strategy/inbox/WP-*.md, находит файлы с status: in_progress | active | awaiting-batch,
...
REGISTRY_FILE="$IWE/DS-my-strategy/docs/WP-REGISTRY.md"
...
_weekplan_wp_ids() {
  local weekplan
  weekplan=$(ls -t "$IWE/DS-my-strategy/current/WeekPlan"*.md 2>/dev/null | head -1)
  [[ -f "$weekplan" ]] || return
