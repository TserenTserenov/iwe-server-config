...
# DS-my-strategy — директория с .git
DS_DIR="$IWE_ROOT/DS-my-strategy"
TOTAL=$((TOTAL + 1))
if [ -d "$DS_DIR" ]; then
    if [ -d "$DS_DIR/.git" ]; then
        FOUND=$((FOUND + 1))
        printf "| \`%s\` | %s | %s |\n" "DS-my-strategy/" "✅" "git-репо (is_git=true)"
    else
        OPTIONAL_MISSING=$((OPTIONAL_MISSING + 1))
        printf "| \`%s\` | %s | %s |\n" "DS-my-strategy/" "⚠️" "директория есть, но не git-репо"
    fi
else
    CRITICAL_MISSING=$((CRITICAL_MISSING + 1))
    printf "| \`%s\` | %s | %s |\n" "DS-my-strategy/" "❌" "директория не найдена"
fi
...
if [ ! -d "$DS_DIR/.git" ]; then
    echo "❌ \`DS-my-strategy\` не git-репо (или директория отсутствует)"
else
    set +e
    ...
    # (1) ~/IWE/FMT-strategy-template/ — отдельная директория (авторская)
    # (2) ~/IWE/FMT-exocortex-template/templates/strategy-skeleton/ — внутри FMT (приезжает через update.sh)
