...
python3 - "$REGISTRY" "$WP_NUM" "$PRIORITY" "$TITLE" "$REPO" "$BUDGET" <<'PYEOF'
import sys
registry_path, wp_num, priority, title, repo, budget = sys.argv[1:7]
...
repo_cell = repo if repo else "DS-my-strategy/inbox/WP-{}-*.md".format(wp_num)
new_row = "| {} | {} | **{}** | ⏳ | {} | {} |\n".format(
