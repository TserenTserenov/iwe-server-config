...
#
# Использование:
#   bash create-wp.sh --title "Название" --budget 5h --priority P3 [--slug slug] [--repo "репо"] [--related "WP-150:dependency,WP-167:продукт"] [--result-kind WorkDone] [--stake "Оснащённость"] [--week-source "прямое поручение пилота"] [--week-result "ожидаемый результат недели"]
...
# --result-kind (WP-481 Ф7): validated against check-result-kind.py, written into frontmatter.
# --stake: manual override for the REGISTRY «Ставка» cell — takes precedence over the
#   auto-computed STAKE_CELL (from --state axes). Backward-compat path for gate-inactive installs.
# --week-source / --week-result: populate WeekPlan «Источник»/«Результат[ недели]» cells instead
#   of the placeholders — merged in from the pre-State-Gate writer (WP-524, 12.08).
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --stake)    STAKE_OVERRIDE="$2"; shift 2 ;;
...
# WP-524 (12.08): manual --stake wins over auto-computed STAKE_CELL — backward-compat
# path for installs without the axes registry (gate inactive, STAKE_CELL stays "—").
if [[ -n "$STAKE_OVERRIDE" ]]; then
  STAKE_CELL="$STAKE_OVERRIDE"
fi
...
if [[ -n "$RESULT_KIND" ]]; then
  FM_STAKE="result_kind: ${RESULT_KIND}
${FM_STAKE}"
fi
...
if [[ -n "$WEEKPLAN" ]]; then
  if ! python3 - "$WEEKPLAN" "$WP_NUM" "$TITLE" "$PRIORITY" "$BUDGET" "$WEEK_SOURCE" "$WEEK_RESULT" <<'PYEOF'
import sys, re
...
if insert_at is None:
    ...
    values_by_name = {
        ...
        "P": priority,
        "Статус": "pending",
        # both spellings: FMT template uses "Результат", live DS-my-strategy
        # schema uses "Результат недели" — column-name lookup below only
        # fills whichever key matches the actual header (WP-524, 12.08).
        "Результат": week_result,
