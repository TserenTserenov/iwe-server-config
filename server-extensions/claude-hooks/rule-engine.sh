...
check_autonomy() {
    ...
    # WP-272 ревизия 2026-08-31 (peer-session -48, консенсус с Codex): голый стем
    # «подтвер» ловил отчётные формы («подтвердил: обе находки…») — FP 100% за день.
    # Инвариант: warn = вопросительные формы с «?» ИЛИ императив подтверждения
    # («подтверди»/«подтвердите» с границей слова через явный класс терминаторов —
    # \b на кириллице в grep ненадёжен). «подтвердил/подтвердишь (буд.)/подтверждаю»
    # без «?» — декларативы, не ловим.
    if echo "$resp" | grep -qE 'ОК\?|применить\?|продолжить\?|записать\?|хотите\?|добавить\?|[Пп]одтверди(те)?([[:space:],.!?:;—-]|$)|[Пп]одтвердишь\?|[Пп]одтвердить\?|[Пп]одтвержда(ешь|ете|ем|ю)\?|[Сс]огласовываем\?'; then
