...
# Оба источника пусты/упали → safe-skip, НО с сигналом в stderr (P6): иначе инцидент
# «стиль не впрыснулся» невидим при диагностике. Поведение (пропуск) не меняем.
[ -n "${FRAGMENT:-}" ] || {
  echo "inject-code-style: WARN оба источника недоступны (диспетчер + p05-core) — стиль не впрыснут" >&2
  echo '{}'; exit 0;
}
