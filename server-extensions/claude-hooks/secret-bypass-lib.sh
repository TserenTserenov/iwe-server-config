...
# WP-544 F12 measure 2: the pattern corpus used to live inline as a single
# python3 -c '...' argument (~2000 lines inside one pair of quotes) -- one
# stray apostrophe in a comment anywhere in that block ended the bash string
# early and broke every protected tool call on the machine (5+ total
# outages, see WP-544 F12). Resolved once at source time so a broken
# secret_patterns.py only fails this one lookup, not string parsing of the
# whole library.
SECRET_BYPASS_LIB_DIR=$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
...
secret_pattern_process() {
  ...
  # WP-544 F12 measure 2 (see SECRET_BYPASS_LIB_DIR above): the pattern
  # corpus used to live here as an inline python3 -c '...' argument; it now
  # lives in secret_patterns.py next to this file, invoked as an ordinary
  # script argument. sys.argv[1] is still the mode, same as before.
  [ -f "$SECRET_BYPASS_LIB_DIR/secret_patterns.py" ] || return 1
  "$SECRET_BYPASS_PYTHON" "$SECRET_BYPASS_LIB_DIR/secret_patterns.py" "$mode"
}
