#!/usr/bin/env python3
# Canonical secret-pattern corpus for the secret-leak guards (WP-544 F12
# measure 2). Invoked by secret_pattern_process() in secret-bypass-lib.sh as
# a standalone file instead of an inline `python3 -c '...'` argument, so a
# broken edit here fails this one lookup instead of breaking bash parsing of
# the whole shared library (see secret-bypass-lib.sh for the incident
# history that motivated the split).
import contextlib
import io
import json
import os
