...
#
# Область действия (решение пир-сессии, WP-530 Ф45, 17.09 — Claude+Kimi):
# эта аренда и её проверка существуют ТОЛЬКО внутри рантайма Claude Code.
# Kimi headless, Codex sandbox, обычные bash-скрипты/раннеры и правка
# пилота напрямую в редакторе идут мимо этого хука полностью — он их не
# видит и не блокирует. Рассматривались два варианта закрыть этот пробел
# (кросс-рантаймовая аренда через свои хуки у Kimi/Codex; hash-chain по
# sha256 между сессиями) — оба признаны либо непропорционально сложными
# (N разных языков/раннеров дублируют один механизм), либо дающими ложное
# чувство защищённости (hash-chain не видит запись, случившуюся ПОСЛЕ
# закрытия сессии — см. разбор в report.md той пир-сессии). Решение:
# не строить кросс-рантаймовую защиту; вместо неё — see write-path-post-verify.sh
# (PostToolUse, тот же манифест), который делает молчаливую потерю видимой
# сразу после записи, независимо от того, кто её вызвал.
