...
# Проверка 2: конкретный D-NNN (drafts/) ИЛИ C-NNN (content video-конвейер РП-514/167,
# extensions/day-open.after.md:376) есть — не только PENDING-плейсхолдер. С 07.08 «Активный
# черновик» штатно заполняется C-NNN видео-сценариями (DS-Tseren-Brand/content/video/) —
# регэксп расширен, чтобы не блокировать легитимный конвейер (WP-507, 09.08).
if ! grep -qE "[DC]-[0-9]+" "$FILE"; then
  echo "  ❌ КРИТИЧЕСКИЙ: Саморазвитие без конкретного D-NNN (drafts/) или C-NNN (видео-сценарии) — COMMIT БЛОКИРОВАН"
  echo "       Требуется: [D-NNN](drafts/D-NNN-тема.md) ИЛИ [C-NNN](DS-Tseren-Brand/content/video/...) и «где остановился»"
  exit 2
fi
...
echo "  ✅ Саморазвитие: D-NNN/C-NNN присутствует, PENDING не обнаружен"
```

- [ ] Саморазвитие: секция `<details>` ИЛИ явная строка в плане **с конкретным D-NNN или C-NNN** и «где остановился»
- [ ] Нет формулировки «саморазвитие X мин» без D-NNN или названия руководства + главы

### Проверка (не блокирующая с 2026-08-16): все РП из priorities.yaml в плане (bug-2026-07-01, разблокировано WP-484 Ф101)
...
> а промпт запрещал LLM добавлять РП вне JSON.
>
> **Изменение 16.08 (прямое поручение пилота):** открытие дня не должно падать из-за
> ошибок с приоритетами — открытие должно быть всегда, замечания идут в сам план. Детекцию
> и запись расхождения теперь делает `day-open-priorities-patch.py` (шаг 4.55 pipeline,
> ДО этой проверки) — пишет находку прямо в секцию «Требует внимания» самого DayPlan, не
> в stdout pre-commit хука (иначе замечание легко потерять после коммита, старый баг
> вернётся в другой форме). Эта проверка теперь только диагностическая сверка: если
...
" 2>/dev/null || true)
  ...
  elif grep -qE "priorities\.yaml отсутствует в таблице" "$FILE"; then
    echo "  ⚠️ $MISSING РП из priorities.yaml отсутствует в плане — уже отмечено в «Требует внимания» (не блокирует)"
  else
    echo "  ⚠️ $MISSING РП из priorities.yaml отсутствует в плане, И patch-скрипт не записал находку — проверь вручную (не блокирует)"
  fi
else
...
- [ ] Каждый `WP-NNN` из `priorities.yaml → today:` присутствует в DayPlan, либо расхождение отмечено в «Требует внимания». Больше не блокирует commit.
...
- [ ] Портфель в работе: формат `~Xh РП всего / ~Yh физ / Плановый мультипликатор ~N.Nx` (переименовано из «Бюджет дня» 2026-09-01, Ф148/WP-484 — число суммирует остаток по всем РП портфеля, не физический лимит одного дня)
...
if grep -q "Разбор заметок" "$FILE"; then
  ...
  # BUGFIX (2026-08-13, WP-484): формулировка заглушки не зафиксирована кодом —
  # рендер 12–13.08 писал «нет неразобранных заметок», точный матч «нет заметок»
  # не срабатывал, и коммит DayPlan блокировался два дня подряд при честном
  # пустом состоянии. Явная альтернатива известных формулировок (не `[а-я]*`:
  # кириллический диапазон ломается под LC_ALL=C).
  if [ "$data_rows" -gt 0 ]; then
    echo "  ✅ Разбор заметок: markdown-ссылки присутствуют ($data_rows шт.)"
  elif echo "$notes_section" | grep -qE '\|[[:space:]]*нет (неразобранных )?заметок[[:space:]]*\|'; then
...
> (36h/36h) для одного дня.
>
> **Изменение 16.08:** та же логика, что у проверки priorities.yaml выше — блокировка
> заменена записью в артефакт. `day-open-priorities-patch.py` (шаг 4.55 pipeline)
> детектирует отсутствие/нереалистичный `phys_hours` и пишет находку в «Требует
...
if [ "$PHYS_ISSUE" -eq 0 ]; then
  ...
  echo "  ⚠️ phys_hours-проблема есть, И patch-скрипт не записал находку — проверь вручную (не блокирует)"
fi
```
...
### 🔴 БЛОКИРУЮЩАЯ ПРОВЕРКА: health-значений в DayPlan быть НЕ должно (резидентность WP-469)

> **Источник:** retrofit-audit WP-469 (24.07.2026) — сон/пульс/плавание коммитились в
> `current/DayPlan {date}.md` и уходили на GitHub 09–19.07. **Обобщено 12.09.2026**
> (пир-сессия Claude+Kimi+Codex, `MC-sessions:2026-09/12/2026-09-12-19-wp469-470-health-owner-node`,
> раунд 3): прежняя проверка ловила только один жёсткий формат строки
> (`**Сон:** Xч Xм`) и пропустила бы любую другую формулировку той же утечки —
> заменена на детерминированный скрипт с более широким словарём терминов и чисел.
> Данные для чата теперь идут через `узел-владелец` (`scripts/lib/health/owner.py`,
> см. `day-open.after.health-owner.md`), а не через старый прямой SQL-хук.
...
if [ -z "$FILE" ] || [ ! -f "$FILE" ]; then
  ...
  bash "$HOME/IWE/DS-my-strategy/scripts/health-residency-guard.sh" "$FILE" || exit 2
fi
```

- [ ] `health-residency-guard.sh` вернул ✅ на текущем DayPlan. Если ❌ — commit заблокирован, найденную строку убрать (значения — только в чате Открытия дня, не в файле).
