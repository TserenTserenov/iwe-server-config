...
## Коды возврата и честность записи (2026-08-09)

Пустой ответ никогда не превращается в запись `[]` — «пилота не спросили» и «у пилота не было смешанных факторов» это разные вещи, и первое не должно попадать в доказательную базу.

| Код | Что случилось | Что делать агенту |
|-----|---------------|-------------------|
| 0 + «✓ Записано» | пилот выбрал факторы | продолжать Week Close |
| 0 + «Пилот пропустил вопрос» | пилот осознанно нажал Enter | продолжать, ничего не записано |
| 2 | запуск без живого ввода (ночной цикл, headless) | **не проглатывать**: в отчёт Week Close строкой «rival_factors за неделю не записаны — нужен живой пилот» |
| 1 | ошибка (нет URL, битая дата, нераспознанный ответ) | поднять в «Требует внимания» |

Причина: с 13.07 по 09.08 (недели W29-W31) записей не появилось — скрипт падал на Linux-хосте на вычислении начала недели ещё до вопроса, а Week Close этого не замечал. Портируемость `date` починена 09.08.
