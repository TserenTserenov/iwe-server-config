...
> **Не дублирует** `week-close.after.routing-map-check.md` (Шаг B1) — тот делает семантическую сверку карты DP.KR.001 с новой практикой (появился ли новый ТИП контента без строки в карте). Этот шаг — структурная целостность уже существующих `routing.yaml`-контрактов (WP-429 Ф6.1, 9/9 Pack на 2026-07-27): резолвится ли `kinds.<KIND>.dir` в реальную директорию, валиден ли `id_pattern` как regex. Протухший путь молча ломает размещение кандидатов того же класса, что инцидент DP.FM.296/301. Touch-триггер (правка застейдженного `routing.yaml`) уже подключён в `pack-lint.sh` `[R0.6]` — этот шаг ловит порчу, случившуюся не через commit (внешняя правка, синк с другого хоста).

**Выполнить:**

```bash
python3 ~/IWE/DS-MCP/knowledge-mcp/scripts/f6_6_routing_map_health.py --scan --json
```
...
### Пробелы рефлексии за неделю (WP-484, горизонт недели)

> **Источник:** 29.07 пилот попросил распространить прямой опрос рефлексии (`day-close.checks.md`) на недельное/месячное закрытие. Здесь не создаётся отдельный «недельный» файл рефлексии — конвейер генерации руководства читает только дневные файлы `history/{месяц}/{дата}-reflection.md` за скользящее окно 7 дней. Задача этого шага — показать пилоту, сколько дней внутри недели остались без рефлексии, чтобы закрыть накопившееся здесь, а не только в момент дневного закрытия.

```bash
bash "$HOME/IWE/scripts/reflection-gap-check.sh" 7
```
