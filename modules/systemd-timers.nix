  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.timers."iwe-scheduler" = {
      ...
      timerConfig = {
        ...
        # 06:20 и 06:30 — последний catch-up Day Open: первый потребляет уже
        # готовый триаж, второй — точную границу его допустимого ожидания.
    ...
    # week-open-orchestrator.sh дублировали друг друга — оба реально исполнялись в
    # ночь 26→27.07. Пилот выбрал week-open-orchestrator.sh каноническим, `wantedBy`
    # снят.
    #
    # ПРОВЕРЕНО ПОВТОРНО 2026-08-03 (WP-484 Ф46, пир-сессия Claude+Codex+Hermes) —
    # оставлено отключённым, НЕ переназначено. Первая гипотеза этой сессии была
    # "week-open-orchestrator.sh не подключён ни к одному триггеру, значит нужно
    # включить этот таймер обратно" — отвергнута после дальнейшей проверки: у
    # `iwe-scheduler.timer` (отдельный, уже активный юнит, 11 пробуждений/сутки) есть
    # СОБСТВЕННЫЙ блок в scheduler.sh (`dispatch()`, "Недельный оркестратор WP-484
    # Ф16.3", условие Вс час≥23), который уже вызывает week-open-orchestrator.sh —
    # и `night_cycle.ticks.week` в params.yaml включён с 31.07 (пир-сессия
    # Kimi+Codex+Hermes), первое боевое окно было в ночь на 02.08. Включение ЭТОГО
    # таймера поверх уже живого пути scheduler.sh воспроизвело бы ровно ту же
    # дупликацию, из-за которой его отключили 27.07 — только с новым партнёром по
    # гонке. Живой путь — scheduler.sh, не этот юнит; правки week-cycle логики
    # делать там, не здесь. Для отката к этому таймеру нужно сначала убрать
    # эквивалентный блок из scheduler.sh, иначе снова получится дублирование.
    ...
    # 08:00/08:10 МСК, не 05:00/05:10 (WP-484: triage-gated Day Open):
    # strategist-morning ("день") реально стартует не только в 03:00, но и
    # повторно до 06:30 (scheduler.sh:428, окно 3:00-6:59) с бюджетом
    # TASK_TIMEOUT_LONG=2400s (40 мин, scheduler.sh:49) на попытку -- то есть
    # легитимный прогон вправе доработать до 07:10 после финальной попытки
    # в 06:30. Верификатор запускается в 08:00: уже после окна создания, но
    # достаточно рано, чтобы сообщить о сбое до рабочего утра. Для недельной
    # ветки (Вс, scheduler.sh:341-380) последняя попытка -- около 23:50
    # + 40 мин таймаут ≈ 00:39, у 05:40 больше четырёх часов запаса. Недельный
    ...
    systemd.timers."iwe-night-cycle-verify-day" = {
      wantedBy    = [ "timers.target" ];
      description = "IWE Ночной верификатор (день) — ежедн 08:00 МСК";
      timerConfig = {
